...
void PerfMap::Enable(PerfMapType type, bool sendExisting)
{
    ...
    if (sendExisting)
    {
        // The AppDomain may not be created yet if PerfMap::Enable is called very early
        // in startup (e.g., via diagnostic server IPC command). In that case, there are
        // no assemblies to iterate, so we can safely skip this section.
